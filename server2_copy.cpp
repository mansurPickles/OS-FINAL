...
/* The mutex protecting numThreads and threadsExited */
...
void addIdsToLookUp(const int& id)
{
    ...
    idsToLookUpList.push_back(idtoAdd);
...
void createInserterThreads()
{
    ...
    for (int i=0; i< NUM_INSERTERS; i++){
        ...
        //update current number of threads

        else {
            if (pthread_mutex_lock(&threadNumMutex)!= 0){
                perror("lock");
                exit(-1);
            }
            numThreads++;

            if (pthread_mutex_unlock(&threadNumMutex)!= 0){
                perror("unlock");
                exit(-1);
            }        }
    }
...
void createFetcherThreads(const int& numFetchers)
{
    ...
    //create the fetcherthreads and push them into vector
    for (int i=0; i< numFetchers; i++){
        pthread_t thread;
        fetcherThreadIds.push_back(thread);
    }

    //run the fetcherThreadFunction
    for (int i=0; i< numFetchers; ++i){
...
void waitForAllThreadsToExit()
{
    ...
    for (int i=0; i< numThreads; i++){
        ...
        //update number of current threads and exited
...
int main(int argc, char** argv)
{
    ...
    if (pthread_mutex_destroy(threadNumMutex)!= 0){
        perror("pthread_mutex_destroy");
    }
